...
clc
clear, close all
[Functions,basepath] = StartupPR;
p = genpath([Functions 'Probe-Reconstruction']); addpath(p), clear p
%% Inputs==================================================================
% Basic Inputs ============================================================
...
N_ev = 1:1:100;                 % #of PC used to reconstruct
...
A = DataExp(dateFile, RunNo_reconst,'Andor',basepath);
...
GSdataSet = GSset(dateFile,RunNos,'basepath',basepath,'Cam','Andor','Mask',mask);
...
grid on; ylabel(sprintf('$\\left<A-P_{re}\\right>$ (counts)'));
...
grid on; ylabel(sprintf('$STD(A-P_{re})$ (counts)'));
