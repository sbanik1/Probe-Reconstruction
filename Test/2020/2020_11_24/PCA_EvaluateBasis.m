...
SaveBasis = true;               % Save Basis [logical]
...
%% Initialize =============================================================
dirSave = 'PCA_Images'; dirCurrent = cd;
if exist(fullfile(cd,dirSave), 'dir') == 0
    mkdir(fullfile(cd,dirSave));
end
saveName_var = ['PCA_var_Date' strrep(dateFile,'_','-') sprintf('_Runs%d-%d',...
    RunNos(1),RunNos(end))];
saveName_reconst = ['PCA_re_Date' strrep(dateFile,'_','-') sprintf('_Run%d',...
    RunNo_reconst)];

%% Determine the PCs ======================================================
PCAdataSet = PCAset(dateFile,RunNos,'basepath',DataPath,'Cam','Andor');
if SaveBasis, save('PCA','PCAdataSet'); end

%% Plot the PC variance ===================================================
figure(1), clf, subplot(2,1,1)
plot(1:1:size(PCAdataSet.Var,1),PCAdataSet.Var,'.','MarkerSize',10); 
grid on; xlabel('PC Number'); ylabel('Variance'); 
...
ylim([10^7 10^10]); grid on; xlabel('PC Number'); ylabel('Variance'); 

if SavePlots
    set(gcf, 'PaperUnits', 'inches','PaperPosition', [0 0 10 10]);
    cd(dirSave); print(saveName_var,'-dpng','-r200'); cd(dirCurrent);        
end
...
% Prepare Square mask and reconstruct =====================================
...
text(0,150,sprintf('$Avg[P_{re}-A]_{out}$ = %0.2f \n$Std[P_{re}-A]_{out}$ = %0.2f',...
    mean(std_out_diff),std(std_out_diff)),'FontSize',14);
subplot(2,2,3)
