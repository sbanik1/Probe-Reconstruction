...
methods
  function obj = DataExp(dateFile, runNos, camera, basepath)
    ...
    elseif strcmp(camera,'FleaH1')==1
      obj.camera = 'FleaH1';
      obj.pixCam = 4.4;
      obj.mag = 0.95; % Calibration: 11/12/19
      obj.bitDepth = 12;
      obj.satCam = 2^obj.bitDepth-1;
      obj.pixAtom = obj.pixCam/obj.mag;
      obj.basepath = basepath;
      ...
      str = {'Image_h','Probe_h','Background_h'};
      obj.atoms = strings(size(obj.runNos));
      obj.probe = strings(size(obj.runNos));
      obj.backg = strings(size(obj.runNos));
      ...
      iMax = max(size(obj.runNos));
      for ii = 1:iMax
          obj.atoms(ii) = strcat(str(1),'_',dateFile,'_',num2str(obj.runNos(ii),'%04d'),'.txt');
          obj.probe(ii) = strcat(str(2),'_',dateFile,'_',num2str(obj.runNos(ii),'%04d'),'.txt');
          obj.backg(ii) = strcat(str(3),'_',dateFile,'_',num2str(obj.runNos(ii),'%04d'),'.txt');
      end

    elseif strcmp(camera,'FleaH4')==1
      obj.camera = 'FleaH4';
      obj.pixCam = 4.4;
      obj.mag = 5.1; % Calibration: 08/11/2020
      obj.bitDepth = 12;
      obj.satCam = 2^obj.bitDepth-1;
      obj.pixAtom = obj.pixCam/obj.mag;
      obj.basepath = basepath;
